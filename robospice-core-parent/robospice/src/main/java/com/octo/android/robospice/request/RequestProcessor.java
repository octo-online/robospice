...
public class RequestProcessor
{
     ...
     *
     * @param context                  the context on which {@link SpiceRequest} will provide their
     *                                 results.
     * @param cacheManager             the {@link CacheManager} that will be used to retrieve
     *                                 requests' result and store them.
     * @param executorService          a custom {@link ExecutorService} that will be used to execute
     *                                 {@link SpiceRequest} .
     * @param requestProcessorListener a listener of the {@link RequestProcessor}, it will be
     *                                 notified when no more requests are left, typically allowing
     *                                 the {@link SpiceService} to stop itself.
     */
    public static RequestProcessor createDefault(final Context context, final CacheManager cacheManager, final ExecutorService executorService, final RequestProcessorListener requestProcessorListener, final NetworkStateChecker networkStateChecker, final RequestListenerNotifier requestListenerNotifier, final SpiceServiceListenerNotifier spiceServiceListenerNotifier)
    {
        Map<CachedSpiceRequest<?>, Set<RequestListener<?>>> mapRequestToRequestListener = Collections.synchronizedMap(new LinkedHashMap<CachedSpiceRequest<?>, Set<RequestListener<?>>>());
        RequestProgressManager requestProgressManager = new RequestProgressManager(requestProcessorListener, mapRequestToRequestListener, requestListenerNotifier, spiceServiceListenerNotifier);
        RequestRunner requestRunner = new DefaultRequestRunner(context, cacheManager, executorService, requestProgressManager, networkStateChecker);
        return new RequestProcessor(mapRequestToRequestListener, requestProgressManager, requestRunner, cacheManager);
    }

    public RequestProcessor(Map<CachedSpiceRequest<?>, Set<RequestListener<?>>> mapRequestToRequestListener,
                            RequestProgressManager requestProgressManager, RequestRunner requestRunner, CacheManager cacheManager)
    {
